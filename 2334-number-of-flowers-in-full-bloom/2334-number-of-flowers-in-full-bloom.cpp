...
public:
    vector<int> fullBloomFlowers(vector<vector<int>>& flowers, vector<int>& people) {
        ...
        for(int i = 0; i<n; i++) {
            ...
            int bloomed = upper_bound(begin(start_time), end(start_time), people[i]) - begin(start_time);
            
            int died = lower_bound(begin(end_time), end(end_time), people[i]) - begin(end_time);
